...
- JavaScript/Express 분석기는 NoSQL Injection도 탐지합니다. MongoDB/Mongoose 쿼리 메서드(`find`/`findOne`/`updateOne`/`deleteMany`/`aggregate` 등)에 **요청 객체 전체**(`User.find(req.query)`처럼 속성 접근 없는 bare `req.query`/`body`/`params`)가 필터로 전달되어 `$ne`/`$gt`/`$where` 같은 연산자 주입이 가능한 경우만 보고합니다. `Array.find(콜백)`이나 타입 지정 필드 읽기(`req.query.id`)는 오탐을 피하기 위해 제외합니다.
